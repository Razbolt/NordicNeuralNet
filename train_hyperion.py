...
def train_model(model, train_loader, val_loader, model_settings,index2word_sv):
    ...
    torch.save({'epochs':epoch+1,'model_state_dict':model.state_dict(),
                ...
                'loss':loss.item()}, 'models/base-hyperion-1.22.pth')
...
if __name__ == '__main__':
    ...
    encoder = Encoder(len(dataset.word2idx_en), embedding_size=256, hidden_size=512, num_layers=4, dropout=0.4)
    decoder = Decoder(len(dataset.word2idx_sv), embedding_size=256, hidden_size=512, num_layers=4, dropout=0.4)
    ...
    wandb_logger = Logger2(f'Hyperion-BaseModel_Seq2Seq_epochs{num_epochs}_b{batch_size}_lr{learning_rate}',project='Machine Translation') # Logger Changed to Logger2
